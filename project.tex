...
\title{CSC446 Project\thanks{\href{https://github.com/parth2324/csc2525-project}{Project Link}}}
\author{Parth Singh}
...
also discuss mesh generation under \textit{gmsh} via two cases. I will be using Dirichlet Boundary Condition, based on
the harmonic function $g(\overrightarrow{x}) = -log(||\overrightarrow{x} - (-7.1, -7.2, -7.3)||)$. \\
...
iterations. This can be particularly helpful when things misbehave around the boundary, such as a singularity at the apex
of a sharp cone. Thus, for case 3, we test the effect of smoothening by comparing unsmoothened mesh based solution and 
10 iterations worth of smoothening based mesh solution.
...
We couldn't compute solution for $MSF = 1/8$ in case 2, given the large volume of the cylinder implying large number of
finite elements. In case 1, we reach numerical limits on computation as we reach $MSF=0.3$, performing worse than $MSF=0.5$;
...
mesh for curvature is generally much harder. In case 3, we observe the benefits of smoothening, an improvement of 
$\approx 10^{-5}$.
